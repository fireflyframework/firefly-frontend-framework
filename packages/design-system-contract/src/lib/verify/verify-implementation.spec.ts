...
describe('verifyDsContracts', () => {
  ...
  it('covers the full inventory (23 primitives + 4 patterns)', () => {
    expect(ALL_CONTRACTS).toHaveLength(27);
    expect(ALL_CONTRACTS.filter((c) => c.category === 'primitive')).toHaveLength(23);
    expect(ALL_CONTRACTS.filter((c) => c.category === 'pattern')).toHaveLength(4);
  });
