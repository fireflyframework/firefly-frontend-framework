...
describe('verifyDsContracts', () => {
  ...
  it('covers the full inventory (23 primitives + 1 pattern)', () => {
    expect(ALL_CONTRACTS).toHaveLength(24);
    expect(ALL_CONTRACTS.filter((c) => c.category === 'primitive')).toHaveLength(23);
    expect(ALL_CONTRACTS.filter((c) => c.category === 'pattern')).toHaveLength(1);
  });
